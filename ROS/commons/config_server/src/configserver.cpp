...
bool ConfigServer::load(const std::string& filename)
{
        ...
        YAML::Node n = YAML::LoadFile(fname.c_str());

        //if(!p.GetNextDocument(n3))
        //{
        ...
        ROS_INFO("Loading configuration file '%s'", fname.c_str());
...
void ConfigServer::insertFromYAML(const YAML::Node& n, const std::string& path)
{
        if(n.Type() == YAML::NodeType::Map)
        {
          //ROS_INFO("MAP");
                ...
                {
                  // ROS_INFO("IT 0");
                    YAML::Node value = it->first;
                    YAML::Node key = it->second;

                    //ROS_INFO("IT 1");
                    insertFromYAML(key, path + "/" + value.as<std::string>());
                }
        }
        ...
        if(n.Type() == YAML::NodeType::Scalar)
        {
          // ROS_INFO("SCALAR");
                ...
                ROS_INFO_STREAM(p.desc.name << " " << p.value);
                p.notify();
        }
}
...
int main(int argc, char** argv)
{
        ...
        CREATE_ROBOLAND_NODE("config_server");
